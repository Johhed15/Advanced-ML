...
The final greedy policy isnt optimal for all states, its choosing a longer path to go over the obstacle then below, even though that the path below is shorter.
...
The values are higher to go above the negative rewards, what could be done is to explore more when choosing the next action, seems like the left bottom corner isnt explored ehough. 
...
We are now avoiding the 5 to get the higher reward in 10, gamma of 0.95 is making the agent to look for long term rewards(10) rather than short term(5). Maybe this is learnt around 15k episodes as there is a big correction there and the average reward goes up at the same time.

## Epsilon= 0.1, alpha=0.1, gamma=0.5, beta=0

If we compare this to the one with epsilon = 0.5 and gamma = 0.5 we see that we don't learn that much and that we are not improving when looking at the moving average reward, the agent found out how to get to 5 then stuck by that. That is what can happen when epsilon is low and you use the best known path 90% of the time. 

## Epsilon= 0.1, alpha=0.1, gamma=0.75, beta=0

We havent explored that much but we move to 10 easier than with gamma =0.5 and epsilon 0.1.

## Epsilon= 0.1, alpha=0.1, gamma=0.95, beta=0

The same as before with epsilon =0.1, we dont expolore thath much, we can se that all the values on the right side of 5 is 0 in the Q-table.
...
With higher probability(higher beta) to slip to left or right when starting in position (1,1) the agent has a problem to explore and learn the optimal path to the wanted state.
