...
smooth_path <-  as.character(unname(apply(smooth_pd,2,which.max)))
...
The smoothed distribution provides the highest accuracy compared to other distributions. This is expected because the filtering distribution only uses information available up to time t for its estimate, whereas smoothing incorporates both the information up to t and all subsequent information. Essentially, smoothing uses all available evidence and operates offline, unlike filtering, which is an online process. The viterbi algorithm always choses the next possible step with the highest probability and not the next step with the highest probability, which when we have noisy data(a bad gps), might not be the best choice.
...
```{r}
# entropy for smooth_dp
ent_sm <- apply(X=smooth_pd, MARGIN=2, FUN=entropy.empirical)
# entropy for filter_pd
ent_f <- apply(X=filt_pd, MARGIN=2, FUN=entropy.empirical)

ent_sm
ent_f
```

The empirical entropy estimator is the maximum likelihood estimator, that is very inefficient and strongly biased when the sample size is small. 
