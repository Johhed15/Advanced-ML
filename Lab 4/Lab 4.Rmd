...
day <- rep(1:365,2190/265) # creating the day variable
...
sekf <- function(sigmaF,ell){
  ...
  class(SquaredExpKernel) <- "kernel" # class to kernel as kernlab expects this
  return(SquaredExpKernel)
} 


evalkernel <- sekf(1,2) # sigmaF = 1 and ell = 2 
...
residuals <- lm(tempdata$temp ~ time+ I(time**2))$residuals

GPfit <- gausspr(time,tempdata$temp,kernel=sekf(20,100),scaled=FALSE,var = var(residuals), variance.model = TRUE)

meanPred <- predict(GPfit, time)
plot(time,tempdata$temp)
